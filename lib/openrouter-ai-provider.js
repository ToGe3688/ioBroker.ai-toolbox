    ...
    async sendRequest(model, messages, max_tokens = 2000, system_prompt = null, temperature = 0.6) {
        ...
        if (response.status != 200) {
            ...
            if (response.status == 402) {
                this.adapter.log.warn(`${this.name}: API Insufficient credits`);
                throw new Error(`API Insufficient credits`);
            }
            if (response.status == 403) {
                this.adapter.log.warn(`${this.name}: API Flagged by moderation filter`);
            ...
            if (response.status == 408) {
                this.adapter.log.warn(`${this.name}: Model request timed out`);
            ...
            if (response.status == 502) {
                this.adapter.log.warn(`${this.name}: API Selected model is down`);
