...
class CustomAiProvider {
	...
	constructor(adapter, bot) {
		...
		this.bot = bot;
		this.adapter.log.debug(`Created new SelfHosted AI provider for assistant: ${bot.bot_name}`);
	}

	/**
     * Sends a request to the self-hosted AI API
	...
	/**
     * Sends the actual HTTP request to the self-hosted AI API
     * @param {Array} messages - The messages to send to the AI
	...
	async sendRequest(messages) {
		...
		if (!url || url == "") {
			this.adapter.log.debug("No api url defined for request");
