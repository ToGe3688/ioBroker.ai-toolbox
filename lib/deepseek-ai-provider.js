...
class DeepseekAiProvider {
     ...
     */
    constructor(adapter) {
        this.name = "Deepseek Provider";
        this.apiToken = adapter.config.deep_api_token;
    ...
    /**
     * Sends the actual HTTP request to the Deepseek AI API
     *
    ...
    async sendRequest(model, messages, max_tokens = 2000, system_prompt = null, temperature = 0.6) {
        ...
        // Delete images because Deepseek does not support multimodal requests
        ...
        if (response.status != 200) {
            if (response.status == 400) {
                ...
                this.adapter.log.warn(`${this.name}: ${data.message}`);
...
module.exports = DeepseekAiProvider;
