        ...
        if (typeof obj === "object" && obj.message) {
            if (obj.command === "tool_request") {
                ...
                for (const bot of this.config.bots) {
                    ...
                    if (
                        bot.bot_name == obj.message.tool ||
                        this.stringToAlphaNumeric(bot.bot_name) == obj.message.tool
                    ) {
                        ...
                        if (obj.message.image_url && obj.message.image_url.trim() != "") {
                            ...
                            if (imageData.success) {
                                ...
                                this.log.warn(
                                    `Request stopped, image fetch failed for tool ${botName} URL: ${
                                        obj.message.image_url
                                    }`,
                                );
                                if (obj.callback) {
                        ...
                        if (obj.callback) {
                            this.sendTo(obj.from, obj.command, response.text, obj.callback);
                        }
                        return;
                    }
                }
            ...
            if (obj.command === "model_request") {
                ...
                for (const model of models) {
                    if (
                        model.value == obj.message.model ||
                        this.stringToAlphaNumeric(model.value) == obj.message.model
                    ) {
                        found = true;
                ...
                const response = await this.startModelRequest(
                    foundModel,
                    messages,
                    obj.message.system_prompt,
                    obj.message.max_tokens,
                    obj.message.temperature,
                );
